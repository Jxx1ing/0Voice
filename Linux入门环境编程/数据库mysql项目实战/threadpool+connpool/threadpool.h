...
{
    ImageTask *imgTask = (ImageTask *)task->user_data;

    char buffer[FILE_IMAGE_LENGTH] = {0}; // 如果buffer小于图片大小会报错
    int img_len = read_image(imgTask->filename, buffer);
    if (img_len <= 0)
    {
        printf("Failed to read image: %s\n", imgTask->filename);
    }

    MYSQL *conn = get_conn(imgTask->conn_pool);
    if (!conn)
    {
        printf("No available DB connection for: %s\n", imgTask->filename);
    }

    printf("Inserting image: %s, len=%d, thread=%lu\n", imgTask->filename, img_len, pthread_self());

    mysql_write_image(conn, buffer, img_len);

    release_conn(imgTask->conn_pool, conn);
